...
import { Bookmark, Share2, Volume2, ArrowUp, ArrowDown, History, Globe, Bot } from 'lucide-react'; // Added Bot icon
...
export function SlangCard({ slang, onUpvote, onDownvote }: SlangCardProps) {
  ...
  return (
    <Card className={cn(
        "w-full transition-shadow duration-300 hover:shadow-lg dark:hover:shadow-primary/20",
        slang.isAIGenerated && "border-dashed border-accent" // Style AI generated cards
    )}>
      <CardHeader>
          ...
          <div>
            <CardTitle className="text-2xl font-bold text-primary flex items-center gap-2">
               ...
               {slang.isAIGenerated && (
                 <TooltipProvider>
                    <Tooltip>
                       ...
                       <TooltipContent>
                         <p>This definition was generated by AI.</p>
                       </TooltipContent>
                    </Tooltip>
                 </TooltipProvider>
               )}
             ...
             <CardDescription className="text-sm text-muted-foreground capitalize flex items-center gap-1">
                 {mainTone} Tone
                 {slang.region !== 'Global' && slang.region !== 'Unknown' && ( // Hide 'Unknown' region
                    <>
             ...
             <Tooltip>
                 <TooltipTrigger asChild>
                    <div className="flex items-center space-x-2">
                        {/* Disable voting buttons for AI generated content */}
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => !slang.isAIGenerated && onUpvote(slang.id)}
                            className={cn(
                                "text-green-600 hover:text-green-700 hover:bg-green-100 dark:hover:bg-green-900/50",
                                slang.isAIGenerated && "opacity-50 cursor-not-allowed"
                            )}
                            disabled={slang.isAIGenerated}
                        >
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => !slang.isAIGenerated && onDownvote(slang.id)}
                            className={cn(
                 ...
                 {slang.isAIGenerated && (
                     <TooltipContent>
                         <p>Voting is disabled for AI-generated entries.</p>
                     </TooltipContent>
                 )}
             </Tooltip>
         </TooltipProvider>
